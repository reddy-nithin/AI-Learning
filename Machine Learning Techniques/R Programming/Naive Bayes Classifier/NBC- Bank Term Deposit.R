#Solution to Assignment on Naive Bayes Classifier (NBC)

#@author Nithineshwar Songala @date 04/02/2025
...
#Build an NBC model on training dataset using 5 predictors:
#age, duration, marital status, housing and previous calls
nbc_model_1 <- naive_bayes(y ~ age + duration + housing + marital + previous, data = data_train)
...
#Build an NBC model 2 on training dataset using 7 predictors:
#age, duration, housing, marital status, previous calls, education status, and job status
nbc_model_2 <- naive_bayes(y ~ age + duration + housing + marital + previous + education + job, data = data_train)
...
#Build an NBC model 3 on training dataset using 6 predictors:
#duration, housing, marital status, previous calls, education status, and job status
nbc_model_3 <- naive_bayes(y ~ duration + housing + marital + previous + education + job, data = data_train)

#Making predictions using nbc_model_3
data_test$class_predicted_3 <- predict(nbc_model_3, data_test[ , c("duration",
                                                                   "housing",
                                                                   "marital",
                                                                   "previous",
                                                                   "education",
...
nbc_model_4 <- naive_bayes(y ~ age + duration + housing + marital + previous + education + job, data = data_train_2)
...
(predictive_accuracy <- sum(diag(confusion_matrix_4))/sum(confusion_matrix_4))